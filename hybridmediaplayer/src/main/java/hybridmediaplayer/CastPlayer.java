...
public final class CastPlayer extends BasePlayer {
    ...
    // Result callbacks.
    private final CastPlayer.StatusListener statusListener;
    private final CastPlayer.SeekResultCallback seekResultCallback;

    // Listeners and notification.
    private final CopyOnWriteArrayList<ListenerHolder> listeners;
    private final ArrayList<CastPlayer.ListenerNotificationTask> notificationsBatch;
    private final ArrayDeque<CastPlayer.ListenerNotificationTask> ongoingNotificationsTasks;
    @Nullable
    private SessionAvailabilityListener sessionAvailabilityListener;

    // Internal state.
    private final CastPlayer.StateHolder<Boolean> playWhenReady;
    private final CastPlayer.StateHolder<Integer> repeatMode;
    @Nullable
    ...
    public CastPlayer(CastContext castContext) {
        ...
        statusListener = new CastPlayer.StatusListener();
        seekResultCallback = new CastPlayer.SeekResultCallback();
        listeners = new CopyOnWriteArrayList<>();
        notificationsBatch = new ArrayList<>();
        ongoingNotificationsTasks = new ArrayDeque<>();

        playWhenReady = new StateHolder<>(false);
        repeatMode = new CastPlayer.StateHolder<>(REPEAT_MODE_OFF);
     ...
     */
    @Nullable
    public PendingResult<RemoteMediaClient.MediaChannelResult> loadItems(
    ...
    public void seekTo(int windowIndex, long positionMs) {
        ...
        if (mediaStatus != null) {
            ...
            notificationsBatch.add(
                    new CastPlayer.ListenerNotificationTask(
                            listener -> listener.onPositionDiscontinuity(DISCONTINUITY_REASON_SEEK)));
        } else if (pendingSeekCount == 0) {
            notificationsBatch.add(new CastPlayer.ListenerNotificationTask(EventListener::onSeekProcessed));
        }
        flushNotifications();
    }
    ...
    private void updateInternalStateAndNotifyIfChanged() {
        ...
        if (wasPlaying != isPlaying) {
            notificationsBatch.add(
                    new CastPlayer.ListenerNotificationTask(listener -> listener.onIsPlayingChanged(isPlaying)));
        }
        ...
        if (currentWindowIndex == C.INDEX_UNSET) {
            if (this.currentWindowIndex == C.INDEX_UNSET)
                return;
     ...
     *
     * <p>This method will only update values whose {@link CastPlayer.StateHolder#pendingResultCallback} matches
    ...
    private void setRepeatModeAndNotifyIfChanged(@Player.RepeatMode int repeatMode) {
        if (this.repeatMode.value != repeatMode) {
            ...
            notificationsBatch.add(
                    new CastPlayer.ListenerNotificationTask(listener -> listener.onRepeatModeChanged(repeatMode)));
        }
    }
